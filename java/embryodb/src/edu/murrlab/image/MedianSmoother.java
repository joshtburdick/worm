...
//			System.out.println("c = " + c + "   i = " + i);
			...
			// XXX this is like what Nucleus.getCorrectedRed()
			// does (I think), but faster
			a.set(j, cd.iNucleus.rwraw - cd.iNucleus.rwcorr3);
	...
	private void setMedians() {
		for(Enumeration<Cell> e = root.depthFirstEnumeration(); e.hasMoreElements(); ) {
			...
			for(int i=0; i<v.size(); i++) {
				...
				// XXX this is just a tiny bit roundabout...
				cd.iNucleus.rwcorr4 = cd.iNucleus.rwraw - (int) (median.get(cd).intValue());
			}
		}
	}
}
