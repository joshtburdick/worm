...
public class AncesTree implements Comparator {
    ...
    public AncesTree(Cell root, NucleiMgr nucleiMgr, int startingIndex, int endingIndex) {
        ...
        iRoot = new Cell(AceTree.ROOTNAME, endingIndex, startingIndex);
        iRoot.setEndTime(1);
    ...
    private void adjustEarlyStartTimes() {
    	...
    	if (t_ABal < 1) return; //cannot adjust
        ...
        int t_MS = x.getTime();
        if (t_MS < 1) return; //cannot adjust as things stand now
    ...
    private void processEntries() {
        ...
        for (int i=iStartingIndex; i <= iEndingIndex; i++) {
            //System.out.println("processEntries, i=" + i);
    ...
    private int processEntry(int i) {
        ...
        //	println("debug");
        //}
        if (iShowDeathsAndDivisions) System.out.println("processEntry: " + i + " ************");
        //System.out.println("processEntry: " + i + " ************");
        i--;
        Vector nuclei;
        Vector nuclei_record = (Vector)iNucleiMgr.getNucleiRecord();
        ...
        if (index == iStartingIndex) {
            for (int j = 0; j < nuclei.size(); j++) {
                ...
                //System.out.println("processEntry:2 " + i + CS + j + CS + n);
                processRootCell(i, n);
            }
        } else {
            ...
            for (int j=0; j < nuclei.size(); j++) {
                ...
                if ((n.successor1 == -1 || n.successor1 == 0) && index < iEndingIndex) {
                    ...
                    if (hashKey != null) c = (Cell)iCells.get(hashKey); //####################
                ...
                if (prevn.successor2 == Nucleus.NILLI) {
                    ...
                    if (iShowDeathsAndDivisions) System.out.println(division(parent.getName(), daughter.getName()));
    ...
    private void makeCellsByNameHash() {
        ...
        boolean b = (namingMethod == Identity.MANUAL);
    ...
    private void processRootCell(int i, Nucleus n) {
        ...
        if (n.status == Nucleus.NILLI) return;
