...
unmix.expr.and.sort.matrix.1 = function(m, r, max.iters=2, eps=1e-15) {
  update.stats = NULL
  x = pos.linear.solve(m, r, max.iters=5, normalize="rows")$X

  for(iter in 1:max.iters) {
    x1 = pos.linear.solve(m, r, max.iters=5, normalize="rows")$X
# browser()
    m1 = t( pos.linear.solve(t(x1), t(r), max.iters=5, normalize="columns")$X )
