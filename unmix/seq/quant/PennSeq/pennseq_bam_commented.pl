...
my $bam;
my $result;
GetOptions('i=s'=>\$inf,'b=s'=>\$bam,'o=s'=>\$result);
if((!($bam))||(!($inf))||(!($result))){
    pod2usage();
}
my %judge;
...
# jtb: modifying this to read multiple BAM files
print "Data Scanning...\n";
...
foreach my $gene (keys %geneinf){
	...
	if($samchr eq $agene[1]){
      ...
      # number the reads in this pair "1" or "2"
			$rdpair{$samname}++;
      # save chromosome of this read
			$rdchr{$samname}=$samchr;
      # rdstart and rdcigar are indexed by "read name", "end" (where "end" is 1 or 2)
...
#    print "2\n";
	...
	if(($rdchr{$rdname} eq $isochr) && (($rdstart{$rdname}{1}>=$isostart[0] && $rdstart{$rdname}{1}<=$isoend[$#isoend]) || ($rdstart{$rdname}{2}>=$isostart[0] && $rdstart{$rdname}{2}<=$isoend[$#isoend]))){
		...
		my @indi2=();
		my @comp=();
    # indicators of whether this fragment overlaps each segment
		for(my $i=0;$i<=$#isostart;$i++){
		    @indi1=(@indi1,0);
		    @indi2=(@indi2,0);
		}
    # indicators of whether this fragment is compatible with each segment
		for(my $i=0;$i<=$#isolist;$i++){
		    @comp=(@comp,1);
		}
    ...
    # check if this fragment overlapped any segment
		my $unique_r1=0;
    ...
    # same thing, for the other end of the read
    ...
    # if either end mapped to a unique isoform...
...
=head1 SYNOPSIS
    ...
    -b comma-separated list of BAM files
