...
time.sort.matrix.row = function(cell.weights, a, b) {
  ...
  # the weights of cells (weighted by overlap)
  w.time.weighted = cell.weights$w * t.overlap
...
time.sort.matrix = NULL
for(i in c(2:18)) {
  time.sort.matrix = rbind(time.sort.matrix,
    time.sort.matrix.row(cell.weights, time.points[i-1], time.points[i]))
}
rownames(time.sort.matrix) = paste("t.", time.points[2:18], sep="")

save(time.sort.matrix, file="git/unmix/image/time_sort_matrix.Rdata")
