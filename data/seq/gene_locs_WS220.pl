...
sub get_exons_old {
  ...
  my $infile = "~/data/wormbase/c_elegans.WS220.annotations.gff2.gz";
  ...
  open IN,
    "cat $dir/ancRNAs_W3PSeq3_ce10.gtf $dir/lincRNAs_W3PSeq3_ce10.gtf |"
    || die;
  while (<IN>) {
    my @a = split /\t/;
    $a[0] =~ s/^chr//;
...
get_exons();
group_exons("merged_exons_WS220.bed", "merged_genes_WS220.gff");
gff_to_bed("merged_genes_WS220.gff", "merged_genes_WS220.bed");
