...
# used for sorting (presumably just for the poster.)
...
marker.on.off = marker.on.off[ c(3:24), ]

# FIXME: use the original clustering
h = hclust(dist(t(cluster.mean[1:20,])))

pdf("git/sort_paper/plot/heatmap/sortMarkers.pdf",
  width=9, height=4)
...
axis(2, at=c(0:16) / 21,
  labels=rownames(marker.on.off)[c(1:17)],
  tick=FALSE, las=2)

# draw the ceh-36 tree above this
# XXX this is really hacky
par(new=TRUE)
plot(1,1, type="n", xlim=c(24,649), ylim=c(1250,20),
